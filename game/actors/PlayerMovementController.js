...
export class PlayerMovementController extends Component {
    ...
    update(delta = 1 / 60) {
        ...
        const isGrounded = this.physics ? this.physics.isBodyGrounded(this.body, 0.18) : false;
        ...
        if (this.dashTimer > 0) {
            ...
            this.body.velocity.x = this.dashVelocity.x;
            this.body.velocity.z = this.dashVelocity.z;
    ...
    triggerDash(direction, speed = 16, duration = 0.14) {
        ...
        this.dashVelocity.set(dashDirection.x * speed, 0, dashDirection.z * speed);
        this.dashTimer = duration;

        if (this.owner) {
            this.owner.group.rotation.y = Math.atan2(dashDirection.x, dashDirection.z);
        }
    }

    _getIsometricMoveDirection(move) {
        const moveDir = new THREE.Vector3();
        if (move.z !== 0) moveDir.addScaledVector(ISO_FORWARD, -move.z);
        if (move.x !== 0) moveDir.addScaledVector(ISO_RIGHT, move.x);
        return moveDir;
    }
