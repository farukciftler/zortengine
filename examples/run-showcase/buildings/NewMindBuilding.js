...
export function buildNewMindBuilding(scene, physics, groundMaterial, position = [0, 0, 0]) {
    ...
    // ── Palette ───────────────────────────────────────────────────────────────
    const navyMat = new THREE.MeshStandardMaterial({ color: 0x0d47a1, roughness: 0.35, metalness: 0.15 });
    const marbleMat = new THREE.MeshStandardMaterial({ color: 0xeceff1, roughness: 0.4 });
    const darkMat = new THREE.MeshStandardMaterial({ color: 0x1a1a2e, roughness: 0.7, metalness: 0.1 });
    const goldAccent = new THREE.MeshStandardMaterial({ color: 0xffc107, emissive: 0x996600, emissiveIntensity: 0.2, roughness: 0.3, metalness: 0.5 });
    const glassMat = new THREE.MeshStandardMaterial({
        color: 0x90caf9, transparent: true, opacity: 0.25,
        roughness: 0.05, metalness: 0.5, side: THREE.DoubleSide, depthWrite: false
    });
    const frameMat = new THREE.MeshStandardMaterial({ color: 0x263238, roughness: 0.4, metalness: 0.8 });
    const steelMat = new THREE.MeshStandardMaterial({ color: 0x78909c, metalness: 0.85, roughness: 0.15 });
    const woodMat = new THREE.MeshStandardMaterial({ color: 0x4e342e, roughness: 0.7 });
    const colMat = new THREE.MeshStandardMaterial({ color: 0xcfd8dc, roughness: 0.4, metalness: 0.4 });
    const screenMat = new THREE.MeshStandardMaterial({ color: 0x0a1628, emissive: 0x0d47a1, emissiveIntensity: 0.7, roughness: 0.1 });
    const chairMat = new THREE.MeshStandardMaterial({ color: 0x212121, roughness: 0.8 });
    const rackGlow = new THREE.MeshStandardMaterial({ color: 0x00e676, emissive: 0x00e676, emissiveIntensity: 1.2 });
    ...
    // ── Interior Floor ───────────────────────────────────────────────────────
    const intFloorMat = new THREE.MeshStandardMaterial({ color: 0xe8eaf6, roughness: 0.3 });
    ...
    [-5.5, 5.5].forEach(cx => {
        const g = new THREE.Mesh(new THREE.PlaneGeometry(3.8, winH), glassMat);
    ...
    const addWorkstation = (x, z, ry = 0) => {
        ...
        // Monitor
        const mon = new THREE.Mesh(new THREE.BoxGeometry(0.7, 0.42, 0.05), screenMat);
        mon.position.set(0, 1.24, -0.2); wg.add(mon);
        const bezel = new THREE.Mesh(new THREE.BoxGeometry(0.74, 0.46, 0.03), new THREE.MeshStandardMaterial({ color: 0x1c1c1c }));
        bezel.position.set(0, 1.24, -0.17); wg.add(bezel);
        // Keyboard
        const kb = new THREE.Mesh(new THREE.BoxGeometry(0.52, 0.02, 0.18), frameMat);
    ...
    const addServerRack = (x, z) => {
        ...
        const cab = new THREE.Mesh(new THREE.BoxGeometry(0.8, 2.0, 0.6), darkMat);
        cab.position.y = 1.0; rack.add(cab);
        for (let su = 0; su < 8; su++) {
            const unit = new THREE.Mesh(new THREE.BoxGeometry(0.72, 0.18, 0.05), darkMat);
            unit.position.set(0, 0.2 + su * 0.22, 0.28); rack.add(unit);
            const led = new THREE.Mesh(new THREE.BoxGeometry(0.04, 0.04, 0.02),
                su % 3 === 0 ? rackGlow : new THREE.MeshStandardMaterial({ color: 0xffa726, emissive: 0xffa726, emissiveIntensity: 1.0 }));
    ...
    const wb = new THREE.Mesh(new THREE.BoxGeometry(4, 1.8, 0.06), new THREE.MeshStandardMaterial({ color: 0xfafafa, roughness: 0.6 }));
